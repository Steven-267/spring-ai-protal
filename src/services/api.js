...
export const chatAPI = {
  ...
  // 删除某条会话
  async deleteChat(type = 'chat', chatId) {
    if (!chatId) return
    const resp = await fetch(`${BASE_URL}/ai/history/${type}/${chatId}`, { method: 'DELETE' })
    if (!resp.ok) throw new Error(`HTTP error! status: ${resp.status}`)
    return true
  },

  // 清空某类型全部会话
  async clearChats(type = 'chat') {
    const resp = await fetch(`${BASE_URL}/ai/history/${type}`, { method: 'DELETE' })
    if (!resp.ok) throw new Error(`HTTP error! status: ${resp.status}`)
    return true
  },
  ...
  // 发送旅游向导 Agent 消息
  async sendTravelMessage(prompt, chatId) {
    try {
      const response = await fetch(`${BASE_URL}/ai/travel?prompt=${encodeURIComponent(prompt)}&chatId=${chatId}`, {
        method: 'GET',
      })
      if (!response.ok) {
        throw new Error(`HTTP error! status: ${response.status}`)
